    ...
    >
      <div className={`overflow-x-auto no-scrollbar shadow-md rounded-xl`}>
        <div className={`max-h-[605px] overflow-y-auto no-scrollbar`}>
          <table
            className={`w-full border-collapse rounded-xl shadow-md whitespace-nowrap`}
          >
            <thead className="sticky top-0 z-10 bg-[#765EA5] text-white">
              <tr>
                <th
                  className={`sticky left-0 z-10 bg-[#765EA5] text-white px-[2rem] py-[1.5rem] text-left text-[1.6rem] font-semibold`}
                >
                  S. No.
                </th>
                  ...
                  >
                    ...
                    {columns.map((column, colIndex) => (
                      ...
                      >
                        ...
                        column.accessor(row) ? (
                          <a
                            href={column.accessor(row)}
                            className={`text-blue-600 underline ml-2`}
                          >
                            {column.accessor(row).split("/").pop()}
                          </a>
                        ) : (
                          column.accessor(row)
                        )}
      ...
      <div
        className={`flex justify-end gap-[2rem] items-center py-[1rem] mt-[1rem]`}
      >
        ...
        <div className={`flex items-center gap-[0.2rem]`}>
          ...
          <div
            className={`overflow-x-auto flex no-scrollbar grow-0 shrink-0 max-w-[200px] mr-[1rem]`}
          >
